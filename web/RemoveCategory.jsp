		...
		<div id="Login">
            ...
            <div id="center">
                ...
                while(result.next())
                {
                    ...
                    if( result.getString(1).equals(request.getParameter("CategoryID")))
                    {
                        categoryName=result.getString(3);
                    }
                }
                ...
                else if(!tree.getChildren(request.getParameter("CategoryID")).isEmpty())
                {
                    out.println("<p align=\"center\">Can not remove this category as it has one or more categorits under it. </p>");
                }
                else if(request.getParameter("checkboxRemove") != null ? (request.getParameter("checkboxRemove").equals("true")? true : false) : false)
                {
                    db.executeUpdate("DELETE FROM category WHERE `CategoryID` ='" + request.getParameter("CategoryID") + "';");
                    tree.removeNode(request.getParameter("CategoryID"));
                    tree.makeMenu(getServletContext().getRealPath("Scripts/Menu/menu.html"));
                    out.println("<p align=\"center\">Category removed successfully. </p>");
                    
                }
                else if(request.getParameter("CategoryID") != null)
                {
                    result = db.executeQuery("select `ItemID` from item WHERE `CategoryID`='" + request.getParameter("CategoryID") + "'");
                    if(result.next())
                    {
                        out.println("<p align=\"center\">Can not remove this category as it has one or more items in it. </p>");
                    }
                    else
                    {
                    %>
                    
                    <form method="post" action="RemoveCategory.jsp">
                        ...
                        <table align="center">
                            <tr>
                                ...
                                <td>
                                    <%
                                    ArrayList<Node> l= tree.getParentNodes(request.getParameter("CategoryID"));
